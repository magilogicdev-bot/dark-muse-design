// Download Ecocity Hero section icons from Figma
...
if (nodeIds.some(id => id.includes('XXXX'))) {
  console.error('❌ Node IDs not configured!');
  console.error('');
  console.error('Please update node IDs in this script:');
  console.error('1. Open Figma Dev Mode for Ecocity design');
  console.error('2. Select each icon element:');
  console.error('   - Scroll Down button (orange)');
  console.error('   - Location button');
  console.error('   - 3D icon button');
  console.error('3. Copy node-id from URL or Dev Mode panel');
  console.error('4. Update nodeIds array in this script');
  console.error('');
  console.error('Figma file: https://www.figma.com/design/PoqoKMYQJFXDIWo8qbsKsj/...?m=dev');
  process.exit(1);
}
...
async function main() {
  console.log('📥 Downloading Ecocity Hero icons from Figma...\n');
  ...
  try {
    ...
    if (response.images) {
      for (let i = 0; i < nodeIds.length; i++) {
        ...
        if (imageUrl) {
          const filePath = path.join(iconsDir, `${iconNames[i]}.png`);
          console.log(`Downloading ${iconNames[i]}...`);
          await downloadFile(imageUrl, filePath);
          console.log(`✓ Saved: ${iconNames[i]}.png`);
        } else {
          console.error(`❌ No URL for node ${nodeId} (${iconNames[i]})`);
        }
      }
      console.log('\n✨ Download complete!');
    } else {
