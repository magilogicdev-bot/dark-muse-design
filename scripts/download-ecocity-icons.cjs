// Download 3 ecocity icon designs from Figma
...
const menuIconsDir = path.join(projectRoot, 'public', 'images', 'menu-icons');

if (!fs.existsSync(menuIconsDir)) {
  fs.mkdirSync(menuIconsDir, { recursive: true });
}

const fileKey = 'PoqoKMYQJFXDIWo8qbsKsj';
const nodeIds = ['1080:7700', '1080:7695', '1080:7696'];
const fileNames = ['design-1.png', 'design-2.png', 'design-3.png'];
...
async function main() {
  console.log('📥 Downloading 3 ecocity icon designs from Figma...\n');
  ...
  try {
    ...
    if (response.images) {
      for (let i = 0; i < nodeIds.length; i++) {
        ...
        if (imageUrl) {
          const filePath = path.join(menuIconsDir, fileNames[i]);
