// Download bank logos from Figma
...
const nodeIds = ['1090:8384', '5023:4', '1090:8364', '1090:8354'];
const bankNames = ['sberbank', 'vtb', 'alfabank', 'promsvyazbank'];
...
async function main() {
  console.log('📥 Downloading bank logos from Figma...\n');
  ...
  try {
    ...
    if (response.images) {
      for (let i = 0; i < nodeIds.length; i++) {
        ...
        if (imageUrl) {
          const logoPath = path.join(banksDir, `${bankNames[i]}-logo.png`);
          console.log(`Downloading ${bankNames[i]} logo...`);
          await downloadFile(imageUrl, logoPath);
          console.log(`✓ Saved: ${bankNames[i]}-logo.png`);
        } else {
