# PowerShell script to download 5 specific designs from Figma
# Usage: .\scripts\download-figma-5-designs.ps1 -Token "your-figma-token"
...
if (-not $Token) {
    ...
    Write-Host "   .\scripts\download-figma-5-designs.ps1 -Token 'your-token'" -ForegroundColor White
    Write-Host ""
...
Write-Host "📥 Downloading 5 designs from Figma..." -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($response.images) {
        ...
        for ($i = 0; $i -lt $nodeIds.Length; $i++) {
            ...
            $filePath = Join-Path $imagesDir "$fileName.png"
